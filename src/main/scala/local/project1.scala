...
object project1 {
	  ...
	  //This is also the master actor
		...
		def receive = {
	    	case "Identify me" =>
	      		...
	      		var sendingItem: Array[Int] = new Array[Int](4)
	      		sendingItem(0) = 4
	      		sendingItem(1) = 10000
	...
	class Worker extends Actor {
		...
		def calculateBitCoinFor(start: Int, nrOfElements: Int, nrOfZeroes: Int): Array[String] = {
				var resarr: Array[String] = new Array[String](1000)
				var k: Int = 0
				val sha = MessageDigest.getInstance("SHA-256");
				var zeroValue = ""
				...
				for(i <- start until (start + nrOfElements)){
					...
					val text = "pradeep.ufl" + returnValue
					...
					if(m.startsWith(zeroValue)){
						...
						resarr(k) = "pradeep.ufl" + returnValue
						k = k + 1
					}
